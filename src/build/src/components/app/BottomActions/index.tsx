...
const BottomActions = () => {
  ...
  const { saveHosts, openAddForm, hosts, editHosts, hasUpdatedHosts, resetHosts } = useApp();
  ...
  const handleDeleteSelected = () => {
    ...
    console.log(`Apagando hosts selecionados: ${updatedHosts.length} hosts restantes`);
    // deselectAll();
  };
  ...
  return (
    <div className="app-bottom-actions draggable non-selectable">
      <Button variant="outline" className="non-draggable" onClick={openAddForm}>
        <BiPlus />
        Adicionar host
      </Button>
      ...
      {hasUpdatedHosts && (
        <Button variant="outline" className="non-draggable ml-auto" onClick={resetHosts}>
          Desfazer alterações
        </Button>
      )}
    </div>
